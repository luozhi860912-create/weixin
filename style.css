...
<div id="wechat-action-panel">
    <button class="wc-side-btn" id="wc-reply-btn" onclick="W.doReply()">
        ${IC.reply}<span class="btn-label">生成回复</span>
    </button>
    <button class="wc-side-btn" id="wc-upload-btn" onclick="W.showUpload()">
        ${IC.upload}<span class="btn-label">上传文件</span>
    </button>
    <button class="wc-side-btn" id="wc-settings-btn" onclick="W.openSettings()">
        ${IC.settings}<span class="btn-label">插件设置</span>
    </button>
</div>
<button id="wechat-toggle-btn" onclick="W.toggle()">
...
<div class="wc-screen" style="display:flex;flex-direction:column;">
    ...
    updateReplyBtn() {
        ...
        if (this.page === 'chat' && this.pending > 0) {
            btn.querySelector('.btn-label').textContent = `生成回复(${this.pending})`;
        } else {
            btn.querySelector('.btn-label').textContent = '生成回复';
        }
    }
    ...
    async scanChatNames() {
        ...
        const body = `<div style="font-size:13px;color:#888;margin-bottom:8px;">聊天记录中发现：</div>
            <div class="friend-list">${names.map(n=>`
                <div class="friend-row" id="scan-${n}" onclick="W.tryAutoAdd('${U.escAttr(n)}')">
...
<div class="wc-screen"><div class="wc-screen-body" style="background:white;">
    ...
    openSettings() {
        ...
        panel.innerHTML = `
<div class="sp-hd"><span>⚙️ WeChatSim 设置</span><button class="sp-close" onclick="W.closeSettings()">✕</button></div>
<div class="sp-bd">
    <div class="sp-group">
        <h4>API 配置</h4>
        ...
        <div class="sp-row"><button class="sp-btn green" onclick="W.fetchModels()">拉取模型列表</button></div>
    ...
    <div class="sp-group">
        ...
        <div style="font-size:12px;color:#888;line-height:1.6;background:#F7F7F7;padding:10px;border-radius:6px;">
            只读取当前酒馆角色的世界书。<br>条目格式（key=角色名）：<br>
            <code>头像：图片链接</code><br>
            <code>照片：链接1,链接2</code><br>
            <code>视频：链接1,链接2</code><br>
            <code>人设：角色描述</code><br>
            <code>聊天风格：说话方式</code><br>
            <code>签名：个性签名</code>
        </div>
    </div>
    <div class="sp-group">
